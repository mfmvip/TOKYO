...
# =================== OWNER - MFMVIP =================== #
ALIVE_NAME = Config.ALIVE_NAME if Config.ALIVE_NAME else "TOKYO"
...
async def logs(event):
    ...
    await tgbot.send_file(
        ...
        "logs.txt",
        reply_to=event.id,
        caption="**Heroku** TOKYO Logs",
        buttons=[
            [Button.url("💠 𝙑𝙄𝙀𝙒 𝙊𝙉𝙇𝙄𝙉𝙀 💠", f"{url}")],
            [Button.url("💠 𝘾𝙍𝘼𝙎𝙃𝙀𝘿 💠", "t.me/MFMVIP")]
        ])
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"TOKYO"))
          )  # pylint: disable=oof
async def settings(event):
    await event.delete()
...
async def settings(event):
    await event.edit("** ✘ ∫ هنا ستجد رابط زد ثـون ورابط التنصيب ايضا**:",
                     buttons=[
                         [Button.url("🔗 رابط السورس🔗", url="https://github.com/MFMVIP/TOKYOpack")],
                         [Button.url("🔗 رابط التنصيب 🔗", url="https://dashboard.heroku.com/new?template=https://github.com/MFMVIP/TOKYOpack/")],
                         [Button.url("✨ الدعم ✨",
                                     url="https://t.me/MFMVIP")]
                     ])
...
async def settings(event):
    if event.sender_id == OWNER_ID:
        ...
        await event.answer("انت لا تستطيع استخدام البوت احصل على بوتك من @MFMVIP", alert=True)


# TOKYO zag - زغرفه اكسس
